...
describe("cli autopilot:validate-summary", () => {
  it("validates a correct summary artifact", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-summary-valid-"));
    ...
    try {
      ...
      const result = await runCli(["autopilot:validate-summary", summaryPath], repoRoot);
      expect(result.status, result.stderr).toBe(0);
      expect(result.stdout).toContain("OK: autopilot summary valid");
      expect(result.stdout).toContain(`sha256=${summary.summarySha256}`);
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
  }, 60_000);

  it("returns machine-readable success in --json mode", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-summary-json-"));
    const summaryPath = path.join(tempDir, "autopilot-run.json");
    ...
    try {
      ...
      const result = await runCli(["autopilot:validate-summary", summaryPath, "--json"], repoRoot);
      expect(result.status, result.stderr).toBe(0);
      ...
      expect(parsed).toMatchObject({
        ...
        computedSha256: summary.summarySha256
      });
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
  ...
  it("uses BIOFLOW_AUTOPILOT_OUT when file arg is omitted", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-summary-env-default-"));
    const summaryPath = path.join(tempDir, "autopilot-run.json");
    ...
    try {
      ...
      const result = await runCli(["autopilot:validate-summary", "--json"], repoRoot, {
  ...
  it("fails when digest does not match", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-summary-mismatch-"));
    const summaryPath = path.join(tempDir, "autopilot-run.json");
  ...
  it("returns machine-readable failure when artifact is a failed run", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-failure-json-"));
    ...
    try {
      ...
      const result = await runCli(["autopilot:validate-summary", artifactPath, "--json"], repoRoot);
      expect(result.status).toBe(1);
      const parsed = JSON.parse(result.stdout) as Record<string, unknown>;
      expect(parsed).toMatchObject({
        ok: false,
  ...
  it("fails when failure artifact digest does not match", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-failure-mismatch-"));
    const artifactPath = path.join(tempDir, "autopilot-run.json");
  ...
  it("fails for schema-invalid summaries", async () => {
    const repoRoot = path.resolve(".");
    const tempDir = await mkdtemp(path.join(tmpdir(), "bioflow-cli-autopilot-summary-schema-"));
    const summaryPath = path.join(tempDir, "autopilot-run.json");

    try {
      const summary = makeSummaryBase();
      const invalid = {
        ...summary,
        progress: { completed: 7, total: 4 },
        summarySha256: sha256Json(summary)
      };
      await writeFile(summaryPath, `${JSON.stringify(invalid, null, 2)}\n`, "utf8");
      ...
      expect(String(parsed.message ?? "")).toContain("progress.completed");
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
  }, 60_000);
